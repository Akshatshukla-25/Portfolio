...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const initPrism = () => {
        ...
        const H = Math.max(0.001, options.height);
        const BW = Math.max(0.001, options.baseWidth);
        const BASE_HALF = BW * 0.5;
        const GLOW = Math.max(0.0, options.glow);
        const NOISE = Math.max(0.0, options.noise);
        const offX = options.offset.x;
        const offY = options.offset.y;
        const SAT = options.transparent ? 1.5 : 1;
        const SCALE = Math.max(0.001, options.scale);
        const HUE = options.hueShift;
        const CFREQ = Math.max(0.0, options.colorFrequency);
        const BLOOM = Math.max(0.0, options.bloom);
        const RSX = 1;
        const RSY = 1;
        const RSZ = 1;
        const TS = Math.max(0, options.timeScale);
        const HOVSTR = Math.max(0, options.hoverStrength);
        const INERT = Math.max(0, Math.min(1, options.inertia));
